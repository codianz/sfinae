...
private:
    ...
    template <typename X, int _ = (&X::funcA, 0)>
...
// こいつが、何やってるのという疑問があると思うが、
// 「std::enable_if_t< has_funcA< T>::value, bool>」が型になって
// 正確には
// 「std::enable_if_t< has_funcA< T>::value, bool> bValue = true」
// であり、bValue という仮引数が省略されている表現になる。
// （気持ち悪いけど、許容されている）
// （テンプレートじゃないが void foo(bool=true) {} なんかも許容される）
